...
class zhihuishu(object):
    ...
    def get_uuid(self):
        ...
        if data['code'] == 403:
            raise KeyError('Check you cookies')
    ...
    def download(self, url, **kwargs):
        raise Exception(
            'In courses, all URLs are same, We can not make sure which video you want now. We will Konw in next day')
    ...
    def download_playlist(self, url, **kwargs):
        ...
        # 循环下载
        for video_name in videos_name_and_url:
            url = videos_name_and_url[video_name]
            ext = url.split('/')[-1].split('.')[-1]
            download_urls([url], video_name, ext, headers=fake_headers,
